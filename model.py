...
# ******************
            ...
            net = tf.layers.conv2d(input,10,3,strides=1)
            net = tf.layers.max_pooling2d(net,2,2)
            print(net.shape)
            shape = net.getshape()
            print(net.shape)
            net = tf.layers.conv2d(net,16,3,stride=1)
            print(net.shape)
            net = tf.layers.conv2d(net,32,3,stride=1)
            print(net.shape)
            net = tf.layers.dense(net, 1024, activation=tf.nn.relu)
            net = tf.layers.dense(net, 600, activation=None)
# ******************
    ...
    def init_sess(self, dirs="model/emotion-orig"):
        """
        初始化会话
        """
    ...
    def train(self):
        ...
        for itr in range(10):
            ...
            print('size of batch:',itr,np.shape(d),np.shape(x),d.dtype)
            ls, _ = self.sess.run(
                [self.loss, self.step],
